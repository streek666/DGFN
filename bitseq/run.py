...
from gfn.loss import F_alpha, G_alpha, Annealing_G_alpha, H_alpha, V_alpha, G_JSD, G_SKL, G_Loss, F_Loss
...
parser.add_argument("--grad_clip", default=1e3, type=float)
parser.add_argument("--dropout", default=0.1, type=float)
parser.add_argument("--batch_size", default=16, type=int)
parser.add_argument("--print_every", default=10, type=int)
parser.add_argument("--flush_every", default=1000, type=int)
parser.add_argument("--cor_every", default=5000, type=int)
parser.add_argument("--print_modes", default=False, action='store_true')

parser.add_argument("--objective", default='tb', type=str)
parser.add_argument("--z_learning_rate", default=0.002, type=float)
parser.add_argument("--subtb_lambda", default=1.5, type=float)
parser.add_argument("--forward_looking", default=False, action='store_true')
...
parser.add_argument("--loss_type", default="G", type=str)
parser.add_argument("--alpha", default=0.0, type=float)
parser.add_argument("--logr_clip", default=0, type=float)
...
parser.add_argument("--logname", default="default", type=str)
...
class Reward():
    def __init__(self, n, k, r, M):
        self.n = n
        self.k = k
        self.r = r
        self.M = [self.construct_grouped_m(m, n, k) for m in M]
        self.ones = []
        for i in range(2**k):
            cnt = 0
            j = i
            while j > 0:
                cnt += j & 1
                j >>= 1
            self.ones.append(cnt)
    
    def construct_grouped_m(self, m, n, k):
        m = int(m, 2)
        ms = [0] * (n//k)
    ...
    def distance(self, m1, m2):
        ...
        return sum([self.ones[m1[i]^m2[i]] for i in range(len(m1)) if m1[i]<(2**self.k)])
    
    def M_distance(self, m):
        return min([self.distance(m, mm) for mm in self.M])
    
    def log_reward(self, batch):
        batch_list = batch.cpu().tolist()
        log_rewards = [-self.M_distance(m) for m in batch_list]
        return torch.tensor(log_rewards)
    
    def reward(self, batch):
        return torch.exp(self.log_reward(batch))
...
def g(t, alpha):
    # return (torch.exp(alpha*t) + torch.exp(-alpha*t) - 2).mean()
    if alpha == 0 :
        return t.pow(2).mean()/2
    return (torch.exp(alpha*t)-alpha*t-1).mean()/(alpha*alpha)

def TB_train_step(model, log_Z, optimizer, Z_optimizer, loss_fn, Rew, args):
    ...
    log_rewards = args.reward_exponent * Rew.log_reward(batch[:, 1:]).to(args.device).detach()
    log_pf = p_forward_sum
    log_pb = log_rewards + p_backward_sum - log_Z.sum()
    loss = loss_fn(log_pf, log_pb)
    loss.backward()
    grad_norm = torch.nn.utils.clip_grad_norm_(optimizer.param_groups[0]['params']+Z_optimizer.param_groups[0]['params'], args.grad_clip)
    optimizer.step()
    Z_optimizer.step()

    assert batch[:, 1:].max() < 2 ** args.k
    return loss.cpu().item(), grad_norm.cpu().item(), batch[:, 1:].cpu()


def DB_train_step(model, optimizer, loss_fn, Rew, args):
    # This code is pretty simple because all trajectories in our graph have the same length.
    model.train()
    optimizer.zero_grad()
    ...
    for i in range(args.n // args.k):
        ...
        if args.forward_looking:
            log_f += args.reward_exponent * Rew.log_reward(batch[:, 1:]).to(args.device).detach()

        if pred_logits is not None:
            log_pf = pred_f + pred_logits
            log_pb = log_f + torch.log(torch.tensor(1 / i)).to(args.device)
            loss += loss_fn(log_pf, log_pb)
    ...
    log_rewards = args.reward_exponent * Rew.log_reward(batch[:, 1:]).to(args.device).detach()
    log_pf = pred_f + pred_logits
    log_pb = log_rewards + torch.log(torch.tensor(1 / (args.n // args.k))).to(args.device)
    loss += loss_fn(log_pf, log_pb)
...
def SubTB_train_step(model, optimizer, loss_fn, Rew, args):
    # This code is pretty simple because all trajectories in our graph have the same length.
    model.train()
    optimizer.zero_grad()
    ...
    for i in range(args.n // args.k):
        ...
        if args.forward_looking:
            log_fs[i] += args.reward_exponent * Rew.log_reward(batch[:, 1:]).to(args.device).detach()
    ...
    log_fs[-1] = args.reward_exponent * Rew.log_reward(batch[:, 1:]).to(args.device).detach()
    ...
    for i in range(log_fs.shape[0]):
        for j in range(i + 1, log_fs.shape[0]):
            ...
            log_pf = log_fs[i, :] + log_pfs[i:j, :].sum(dim=0)
            log_pb = log_fs[j, :] + log_pbs[i+1:j+1, :].sum(dim=0)
            loss += lmbd * loss_fn(log_pf, log_pb)
            # loss += lmbd * g( - log_fs[i, :] - log_pfs[i:j, :].sum(dim=0) + log_fs[j, :] + log_pbs[i+1:j+1, :].sum(dim=0), args.alpha)
...
def main(args):
    ...
    Rew = Reward(args.n, args.k, args.reward_exponent, M) 
    ...
    if args.objective == "softdqn":
        ...
        if args.loss_type == 'G':
            loss_fn = G_Loss(G_alpha(args.alpha), clip=args.logr_clip)
        elif args.loss_type == 'H':
            loss_fn = G_Loss(H_alpha(args.alpha), clip=args.logr_clip)
        elif args.loss_type == 'V':
            loss_fn = G_Loss(V_alpha(args.alpha), clip=args.logr_clip)
        elif args.loss_type == 'JSD':
            loss_fn = G_Loss(G_JSD(args.alpha), clip=args.logr_clip)
        elif args.loss_type == 'SKL':
            loss_fn = G_Loss(G_SKL(args.alpha), clip=args.logr_clip)
        elif args.loss_type == 'anneal':
            loss_fn = G_Loss(Annealing_G_alpha(args.num_iterations), clip=args.logr_clip)
        else:
            loss_fn = F_Loss(F_alpha(args.alpha), clip=args.logr_clip)
    
    for it in tqdm(range(args.num_iterations + 1)):
        progress = float(it) / args.num_iterations
        if args.objective == "tb":
            loss, grad, batch = TB_train_step(model, log_Z, optimizer, Z_optimizer, loss_fn, Rew, args)
        elif args.objective == "db":
            loss, grad, batch = DB_train_step(model, optimizer, loss_fn, Rew, args)
        elif args.objective == "subtb":
            loss, grad, batch = SubTB_train_step(model, optimizer, loss_fn, Rew, args)
        ...
        if args.objective != "softdqn" and args.loss_type == "anneal":
            loss_fn.g.step()
        ...
        if it > 0 and it % args.print_every == 0:
            print(f"{it}/{args.num_iterations}, loss: {loss}, grad: {grad}, modes: {sum(modes), sum(cum_modes)}, avg reward: {avg_reward / args.print_every}, log_Z: {log_Z.sum().cpu().item()}")
            with open(args.logname, "a") as ff:
                ff.write(f"{it}/{args.num_iterations}, loss: {loss}, grad: {grad}, modes: {sum(modes), sum(cum_modes)}, avg reward: {avg_reward / args.print_every}, log_Z: {log_Z.sum().cpu().item()}\n")
            if it % args.flush_every == 0:
                modes = [False] * len(M)
            avg_reward = 0.0

        if it > 0 and it % args.cor_every == 0:
            if args.print_modes:
                print("found modes:")
                for m in range(len(M)):
                    if modes[m]:
