...
from gfn.loss import F_alpha, G_alpha, Annealing_G_alpha, Annealing_Mix_KL, H_alpha, G_Loss, F_Loss
...
parser.add_argument("--logr_clip", default=10, type=float)
...
# @torch.jit.script
def tb_lambda_loss(P_F, P_B, F, R, traj_lengths, Lambda, loss_fn):
    cumul_lens = torch.cumsum(torch.cat([torch.zeros(1, device=traj_lengths.device), traj_lengths]), 0).long()
    total_loss = torch.zeros(1, device=traj_lengths.device)
    ...
    for ep in range(traj_lengths.shape[0]):
        ...
        for i in range(T):
            for j in range(i, T):
                ...
                log_pf, log_pb = F[offset + i], F[offset + min(j+1, T-1)] * flag + R[ep] * (1-flag)
                for k in range(i, j + 1):
                    # acc += P_F[offset + k] - P_B[offset + k]
                    log_pf += P_F[offset + k]
                    log_pb += P_B[offset + k]
                # total_loss += acc.pow(2) * Lambda ** (j - i + 1)
                total_loss += loss_fn(log_pf, log_pb) * Lambda ** (j - i + 1)
...
def train_model_with_proxy(args, model, proxy, dataset, loss_fn, num_steps=None, do_save=True):
    ...
    if do_save:
        exp_dir = f'{args.save_path}/{args.loss_type}_{str(args.alpha).zfill(3)}_{args.seed+10}/'
    ...
    def save_stuff(iter):
        ...
        pickle.dump(dataset.sampled_mols,
                    gzip.open(f'{exp_dir}/' + '_sampled_mols.pkl.gz', 'wb'))
    ...
    for i in range(num_steps):
        ...
        if args.objective == 'fm':
            ...
            if args.objective == 'tb':
                ...
                loss = tb_lambda_loss(logits, torch.log(1/n), mol_out_s[:, 1], torch.log(traj_r), lens, Lambda, loss_fn)
            opt.zero_grad()
            loss.backward()
            ...
            if args.clip_grad > 0:
                torch.nn.utils.clip_grad_value_(model.parameters(),
                                               args.clip_grad)
        opt.step()
        if args.loss_type in ['anneal','anneal2']:
            loss_fn.g.step()
...
def main(args):
    ...
    device = torch.device('cuda')
    print(args)
    set_seed(args.seed, is_cuda=True)
    ...
    elif args.loss_type == 'anneal2':
        loss_fn = G_Loss(Annealing_Mix_KL(args.num_iterations), clip=args.logr_clip)
    else:
    ...
    train_model_with_proxy(args, model, proxy, dataset, loss_fn, do_save=True)
    print('Done.')
