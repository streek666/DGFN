...
class TBGFlowNet(TrajectoryBasedGFlowNet):
    ...
    def loss(self, trajectories: Trajectories, on_pf=True) -> TT[0, float]:
        ...
        """
        log_pfs, log_pbs, log_rewards = self.get_trajectories_scores(trajectories)
        loss = self.loss_fn(log_pfs+self.logZ, log_pbs+log_rewards, on_pf=on_pf)
        if torch.isnan(loss):
...
class LogPartitionVarianceGFlowNet(TrajectoryBasedGFlowNet):
    ...
    def loss(self, trajectories: Trajectories, on_pf=True) -> TT[0, float]:
        ...
        """
        log_pfs, log_pbs, log_rewards = self.get_trajectories_scores(trajectories)
        scores = (log_pfs + log_pbs - log_rewards).detach()
        loss = self.loss_fn(log_pfs, log_pbs+log_rewards-scores.mean(), on_pf=on_pf)
        if torch.isnan(loss):
            raise ValueError("loss is NaN.")
