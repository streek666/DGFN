...
class SubTBGFlowNet(TrajectoryBasedGFlowNet):
    ...
    def loss(self, trajectories: Trajectories, on_pf=True) -> TT[0, float]:
        # Get all scores and masks from the trajectories.
        pfs, pbs, flattening_masks = self.get_scores(trajectories)
        flattening_mask = torch.cat(flattening_masks)
        all_pfs = torch.cat(pfs, 0)
        all_pbs = torch.cat(pbs, 0)
        ...
        if self.weighting == "equal_within":
            ...
            contributions = contributions.to(is_done.device)
            ...
            contributions = torch.zeros_like(all_pfs)
            indices = (
            ...
            return self.loss_fn(pfs[0][~flattening_masks[0]], pbs[0][~flattening_masks[0]], on_pf=on_pf)
            ...
            contributions = contributions.to(is_done.device) / per_trajectory_denom / len(trajectories)
            ...
            per_length_losses = torch.stack(
                [
                    self.loss_fn(pf[~flattening_mask], pb[~flattening_mask], on_pf=on_pf)
                    for pf, pb, flattening_mask in zip(pfs, pbs, flattening_masks)
                ]
            )
        ...
        losses = flat_contributions * self.loss_fn(all_pfs[~flattening_mask], all_pbs[~flattening_mask], on_pf=on_pf, reduce='none')
        return losses.sum()
