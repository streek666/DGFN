...
class DBGFlowNet(PFBasedGFlowNet):
    ...
    def get_scores(
        ...
        return (valid_log_pf_actions, log_pb_actions, preds, targets)

    def loss(self, transitions: Transitions, on_pf=True) -> TT[0, float]:
        """Detailed balance loss.

        The detailed balance loss is described in section
        3.2 of [GFlowNet Foundations](https://arxiv.org/abs/2111.09266)."""
        _, _, pf, pb = self.get_scores(transitions)
        loss = self.loss_fn(pf, pb, on_pf=on_pf)
...
class ModifiedDBGFlowNet(PFBasedGFlowNet):
    ...
    def get_scores(self, transitions: Transitions) -> TT["n_trajectories", torch.float]:
        ...
        preds = all_log_rewards[:, 0] + valid_log_pf_actions - valid_log_pf_s_exit
        targets = all_log_rewards[:, 1] + valid_log_pb_actions - valid_log_pf_s_prime_exit
    ...
    def loss(self, transitions: Transitions, on_pf=True) -> TT[0, float]:
        ...
        pf, pb = self.get_scores(transitions)
        self.loss_fn(pf, pb, on_pf=on_pf)
